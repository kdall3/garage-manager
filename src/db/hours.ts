...
export async function addShift(employee_id: number, reg_plate: string, start_time: Date, end_time: Date): Promise<AddShiftResult> {
    ...
    await db.query('INSERT INTO hours (employee_id, reg_plate, start, end) VALUES (?, ?, ?, ?)', [employee_id, reg_plate, start_time, end_time]);
    return 'OK';
}